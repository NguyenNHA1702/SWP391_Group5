...
<html lang="en">
    ...
    if (confirm('Are you sure you want to delete this conversation?')) {
        fetch('<%= request.getContextPath()%>/MessageServlet?action=deleteConversation&userId=' + userId + '&otherUserId=' + otherUserId, {
            method: 'POST'
        })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'Conversation deleted') {
                ...
                document.getElementById('messageArea').innerHTML = '<div class="p-4 text-gray-500">Select a conversation to start</div>';
                selectedUserId = null;
                document.getElementById('chatWith').textContent = 'Select a conversation';
            } else {
